...
class SnapshotController extends BaseController
{
    public function store(Request $request): never
    {
        ...
        try {
            ...
            $this->back('快照创建成功。', 'success', $redirectTo);
        } catch (\Throwable $e) {
            $this->back('快照创建失败：' . $e->getMessage(), 'error', $redirectTo);
        }
    }
    ...
    public function revert(Request $request): never
    {
        ...
        try {
            ...
            $this->back('快照回滚成功。', 'success', $redirectTo);
        } catch (\Throwable $e) {
            $this->back('快照回滚失败：' . $e->getMessage(), 'error', $redirectTo);
        }
    }
    ...
    public function delete(Request $request): never
    {
        ...
        try {
            ...
            $this->back('快照删除成功。', 'success', $redirectTo);
        } catch (\Throwable $e) {
            $this->back('快照删除失败：' . $e->getMessage(), 'error', $redirectTo);
        }
    }
}
